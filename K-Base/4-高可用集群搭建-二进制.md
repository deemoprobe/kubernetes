...
# containerd的CLI是ctr，操作和docker命令类似
...
# 配置crictl客户端连接的运行时socket接口（指向containerd's GRPC server：/run/containerd/containerd.sock）
...
```bash
# k8s-master01上执行，更改calico中Pod网段为自己的，由于policy/v1beta1将被废弃，改为policy/v1
